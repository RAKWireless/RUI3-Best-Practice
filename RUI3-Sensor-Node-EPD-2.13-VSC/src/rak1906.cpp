...
uint8_t
rak1906::readByte(const uint8_t addr) const
{
...
rak1906::init()
{
    ...
    uint8_t         workRegister = readByte(RAK1906_CONTROL_MEASURE_REGISTER);	// Read 
										// the 
										// control 
										// measure
    putData(RAK1906_CONTROL_MEASURE_REGISTER, (uint8_t) (workRegister | 1));	// Trigger 
										// 1st 
										// measurement
    setOversampling(TemperatureSensor, Oversample16);	// Use enumerated
							// type values
    setOversampling(HumiditySensor, Oversample16);	// Use enumerated
							// type values
    setOversampling(PressureSensor, Oversample16);	// Use enumerated
							// type values
    setIIRFilter(IIR4);		// Use enumerated type values
    setGas(320, 150);		// 320�c for 150 milliseconds
...
rak1906::update()
{
    ...
    int64_t         var1,
                    ...
                    var5,
                    var6,
                    temp_scaled;	// Work variables
    uint32_t        adc_temp,
                    adc_pres;	// Raw ADC temperature and pressure
    uint16_t        adc_hum,
                    adc_gas_res;	// Raw ADC humidity and gas
    waitForReadings();
    getData(RAK1906_STATUS_REGISTER, buff);	// read all 15 bytes in
						// one go
    adc_pres = (uint32_t) (((uint32_t) buff[2] << 12) | ((uint32_t) buff[3] << 4) | ((uint32_t) buff[4] >> 4));	// put 
														// the 
														// 3 
														// bytes 
														// of 
														// Pressure
    adc_temp = (uint32_t) (((uint32_t) buff[5] << 12) | ((uint32_t) buff[6] << 4) | ((uint32_t) buff[7] >> 4));	// put 
														// the 
														// 3 
														// bytes 
														// of 
														// Temperature
    adc_hum = (uint16_t) (((uint32_t) buff[8] << 8) | (uint32_t) buff[9]);	// put 
										// the 
										// 2 
										// bytes 
										// of 
										// Humidity
    adc_gas_res = (uint16_t) ((uint32_t) buff[13] << 2 | (((uint32_t) buff[14]) >> 6));	// put 
											// the 
											// 2 
											// bytes 
											// of 
  ...
  *******************************/
    var1 = ((int32_t) adc_temp >> 3) - ((int32_t) _T1 << 1);	// Perform 
								// calibration/adjustment
    var2 = (var1 * (int32_t) _T2) >> 11;	// of Temperature values
						// according
    var3 = ((var1 >> 1) * (var1 >> 1)) >> 12;	// to formula defined by
						// Bosch
  ...
  *******************************/
    ...
    var1 =
	(((((var1 >> 2) * (var1 >> 2)) >> 13) *
	  ((int32_t) _P3 << 5)) >> 3) + (((int32_t) _P2 * var1) >> 1);
    var1 = var1 >> 18;
    ...
    var1 =
	((int32_t) _P9 *
	 (int32_t) (((_Pressure >> 3) * (_Pressure >> 3)) >> 13)) >> 12;
  ...
  **********************/
	...
	((int32_t) _H2 *
	 (((temp_scaled * (int32_t) _H4) / ((int32_t) 100)) +
	  (((temp_scaled *
	     ((temp_scaled * (int32_t) _H5) / ((int32_t) 100))) >> 6) /
...
uint8_t
rak1906::setOversampling(const uint8_t sensor, const uint8_t sampling)
{
    if (sensor >= UnknownSensor || (sampling != UINT8_MAX && sampling >= UnknownOversample)) {	// return 
												// an 
												// error 
												// if 
												// sensor 
	...
	{
	    tempRegister = readByte(RAK1906_CONTROL_HUMIDITY_REGISTER);	// Read 
									// the 
									// register 
									// contents
	    if (sampling == UINT8_MAX) {	// If we just want to read 
						// values

		returnValue = tempRegister & ~RAK1906_HUMIDITY_MASK;	// Set 
									// return 
									// value
	    } else {
		tempRegister &= RAK1906_HUMIDITY_MASK;	// Mask bits to 0
		tempRegister |= sampling;	// Add in the sampling
						// bits
		putData(RAK1906_CONTROL_HUMIDITY_REGISTER, (uint8_t) tempRegister);	// Update 
											// humidity 
											// bits 
											// 0:2
	    }			// if-then return current value or set new 
				// value
	    break;
	}			// of HumiditySensor
    case PressureSensor:	// Set the pressure oversampling
	{
	    tempRegister = readByte(RAK1906_CONTROL_MEASURE_REGISTER);	// Read 
									// the 
									// register 
									// contents
	    if (sampling == UINT8_MAX) {	// If we just want to read 
						// the values
		returnValue = (tempRegister & ~RAK1906_PRESSURE_MASK) >> 2;	// Set 
										// return 
										// value
	    } else {
		tempRegister &= RAK1906_PRESSURE_MASK;	// Mask bits to 0
		tempRegister |= (sampling << 2);	// Add in sampling 
							// bits at offset
		putData(RAK1906_CONTROL_MEASURE_REGISTER, (uint8_t) tempRegister);	// Update 
											// register
	    }			// if-then return current value or set new 
				// value
	    break;
    ...
    case TemperatureSensor:	// Set the temperature oversampling
	{
	    ...
	    if (sampling == UINT8_MAX) {	// If we just want to read 
						// the values
		returnValue = (tempRegister & ~RAK1906_TEMPERATURE_MASK) >> 5;	// Set 
										// return 
										// value
	    } else {
		tempRegister &= RAK1906_TEMPERATURE_MASK;	// Mask
								// bits to 
								// 0
		tempRegister |= (sampling << 5);	// Add in the
							// sampling bits
							// at offset
		putData(RAK1906_CONTROL_MEASURE_REGISTER, (uint8_t) tempRegister);	// Update 
											// humidity 
...
uint8_t
rak1906::setIIRFilter(const uint8_t iirFilterSetting) const
{
    waitForReadings();		// Ensure any active reading is finished
    uint8_t         returnValue = readByte(RAK1906_CONFIG_REGISTER);	// Get 
									// control 
									// register 
									// byte 
									// contents
	...
	returnValue |= (iirFilterSetting & 0x07) << 2;	// use 3 bits of
							// iirFilterSetting
	putData(RAK1906_CONFIG_REGISTER, returnValue);	// Write new
							// control
							// register value
    }				// if the value is to be changed //
    returnValue = (returnValue >> 2) & 0x07;	// Extract IIR filter
...
bool
rak1906::setGas(uint16_t GasTemp, uint16_t GasMillis) const
{
    uint8_t         gasRegister = readByte(RAK1906_CONTROL_GAS_REGISTER2);	// Read 
										// current 
										// register 
										// values
    if (GasTemp == 0 || GasMillis == 0) {
	// If either input variable is zero //
	putData(RAK1906_CONTROL_GAS_REGISTER1, (uint8_t) 0x08);	// Turn
								// off gas 
								// heater
	putData(RAK1906_CONTROL_GAS_REGISTER2, (uint8_t) (gasRegister & 0xEF));	// Turn 
										// off 
										// gas 
										// measurements
    } else {
	putData(RAK1906_CONTROL_GAS_REGISTER1, (uint8_t) 0);	// Turn
								// off
								// heater
								// bit to
								// turn on
	...
	var2 = (_G1 + 784) * (((((_G2 + 154009) * GasTemp * 5) / 100) + 3276800) / 10);	// Issue 
											// #26
	var3 = var1 + (var2 / 2);
	...
	putData(RAK1906_CONTROL_GAS_REGISTER1, (uint8_t) 0);	// then
								// turn
								// off gas 
								// heater
    ...
    }				// of if-then-else turn gas measurements
				// on or off
    return true;
}

float
altitude(const int32_t press, const float seaLevel)
{
    static float    Altitude;
    Altitude = 44330.0 * (1.0 - pow(((float) press / 100.0) / seaLevel, 0.1903));	// Convert 
											// into 
											// meters
    return (Altitude);
}				// of method altitude()
...
void
rak1906::getCalibration()
{
 /*************************************
  ** Temperature related coefficients **
  *************************************/
    uint8_t         coeff_arr1[RAK1906_COEFF_SIZE1] = { 0 };	// Define
								// temp
								// array 1 
								// and
								// initialize 
								// with 0x
    uint8_t         coeff_arr2[RAK1906_COEFF_SIZE2] = { 0 };	// Define
								// temp
								// array 2 
								// and
								// initialize 
								// with 0s
    getData(RAK1906_COEFF_START_ADDRESS1, coeff_arr1);	// Split reading
							// registers into
							// 2
    getData(RAK1906_COEFF_START_ADDRESS2, coeff_arr2);	// one 25 bytes
							// and the other
							// 16
    _T1 =
	(uint16_t) (CONCAT_BYTES
		    (coeff_arr2[RAK1906_T1_MSB_REG],
		     coeff_arr2[RAK1906_T1_LSB_REG]));
    _T2 =
	(int16_t) (CONCAT_BYTES
		   (coeff_arr1[RAK1906_T2_MSB_REG],
  ...
  ************************************/
    ...
    _G2 =
	(int16_t) (CONCAT_BYTES
		   (coeff_arr2[RAK1906_GH2_MSB_REG],
...
void
rak1906::waitForReadings() const
{
    while (measuring()) {
    }				// loop until any active measurment is
				// complete
}				// of method waitForReadings

bool
rak1906::measuring() const
{
    bool            result(false);
...
void
rak1906::triggerMeasurement() const
{
    uint8_t         workRegister = readByte(RAK1906_CONTROL_MEASURE_REGISTER);	// Read 
										// the 
										// control 
